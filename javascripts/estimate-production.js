			...
			chrome.scripting.executeScript(
				{
					...
					function: () => {
						const datetime = document.querySelector(
							'[id="reportWrapper"] [class="header "] [class="time"] [class="text"]',
						).innerText;
						...
						const production = document.querySelectorAll(
							'[class="additionalInformation"] [class="inlineIconList resourceWrapper"]',
						)[1].children[1].children[1].innerText;
						return {
							datetime,
							timestamp,
...
document.addEventListener("DOMContentLoaded", () => {
	...
	chrome.storage.local.get([lsk_report1, lsk_report2, lsk_productionPerHour], (result) => {
		...
		if (result[lsk_productionPerHour])
			document.getElementById("report-result").innerText = result[lsk_productionPerHour].toFixed(0);
	});
});
