			...
			if( neFL ) {
				...
				if( aCCs[i][3].e.indexOf('oasis') != -1 && typeof aCCs[i][3].uid == "undefined" ) {
					...
					if( animX.snapshotLength > 0 ) {
                        ...
                        var backgroundColor = res < 11000 && res>3900 && ratio> 1.9?'lightGreen':'unset'
						tBody.rows[i].cells[3].appendChild($em('div',[$em('div',[heroxp],[['class','body']])],[['id','reportWrapper'],['style','font-weight:bold;']]));
						tBody.rows[i].cells[4].appendChild($em('div',[$em('div',[`${res}|${ratio}`],[['class','body']])],[['id','reportWrapper'],['style',`font-weight:bold;background-color:${backgroundColor}`]]));
